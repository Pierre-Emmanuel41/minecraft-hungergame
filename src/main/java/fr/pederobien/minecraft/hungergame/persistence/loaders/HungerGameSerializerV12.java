...
public class HungerGameSerializerV12 extends AbstractHungerGameSerializer {

	public HungerGameSerializerV12(IPlatformPersistence<IBorder> borderPersistence) {
		super(1.2, borderPersistence);
	}
	...
	public boolean deserialize(IHungerGame element, Element root) {
		...
		setItemOnPlayerKills(element, root);

		// Getting configuration rules
		setRules(element, root);
		return true;
	}
	...
	public boolean serialize(IHungerGame element, Element root) {
		...
		Element isUhc = createElement(HungerGameXmlTag.IS_UHC);
		isUhc.appendChild(createTextNode(element.getUhc().toString()));
		...
		Element itemOnPlayerKills = createElement(HungerGameXmlTag.ITEM_ON_PLAYER_KILLS);
		itemOnPlayerKills.appendChild(createTextNode(normalizeMaterial(element.getItemOnPlayerKills().get().getType())));
		root.appendChild(itemOnPlayerKills);

		Element rules = createElement(HungerGameXmlTag.RULES);
		for (IRule<?> rule : element.getRules().toList()) {
			Element ruleElement = createElement(HungerGameXmlTag.RULE);
			setAttribute(ruleElement, HungerGameXmlTag.NAME, rule.getName());
			setAttribute(ruleElement, HungerGameXmlTag.CURRENT, rule.getValue());
